...
```
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌───────────┐    ┌──────┐    ┌────────┐
│ Poll API │───▶│ Collect  │───▶│ Dedup    │───▶│ Aggregate │───▶│ Rank │───▶│ Submit │
│ (x10)    │    │ Events   │    │ Events   │    │ Scores    │    │      │    │        │
└──────────┘    └──────────┘    └──────────┘    └───────────┘    └──────┘    └────────┘
    5s gap          15 raw        9 unique       3 participants   sorted     POST JSON
  per poll         events         events         totalled         desc       → HTTP 201
```
...
| Duplicates discarded | 6 |
| Unique participants | 3 |
| Grand total score | 1365 |
| Submission status | HTTP 201 — accepted |
...
Note: the full run takes about 45 seconds - 10 polls with a 5 second mandatory delay between each one.
...
The quiz API is designed to return overlapping data - the same event can show up in multiple poll windows. If you naively sum all scores across all polls without deduplicating, you end up inflating totals significantly.

The fix is straightforward: before aggregating anything, assign each event a composite key of `roundId + "|" + participant`. This uniquely identifies a participant's score for a specific round. A `HashSet` tracks which keys have already been seen, and any event whose key has already appeared is discarded. Only the first occurrence is kept. This reduced the raw event count from 15 down to 9 in the actual run above.
...
Without dedup:
  Poll 1 → Alice R1 +10
  Poll 3 → Alice R1 +10 (duplicate)
  Total = 20 ❌

With dedup:
  Poll 1 → Alice R1 +10
  Poll 3 → Alice R1 → already seen, skip
  Total = 10 ✓
```
